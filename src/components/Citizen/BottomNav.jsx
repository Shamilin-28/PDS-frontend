...
const BottomNav = ({ active, setView }) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 px-6 py-3 flex justify-between items-center z-50 rounded-t-[30px] shadow-[0_-10px_30px_-15px_rgba(0,0,0,0.1)]">
      {navItems.map((item) => {
        ...
        return (
          <button
            ...
            className={`flex flex-col items-center gap-1 transition-all duration-300 ${
              isActive ? 'text-emerald-500 transform -translate-y-1' : 'text-slate-400'
            }`}
          >
